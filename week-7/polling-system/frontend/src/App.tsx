...
function App() {
  ...
  const [sharePollLink, setSharePollLink] = useState<string>();
  ...
  async function createPoll(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.navigator.clipboard.writeText(pollLink);
      setSharePollLink(pollLink);
    } catch (error) {
  ...
  return (
    <div>
      <ShareDialog
        isOpen={sharePollLink ? true : false}
        link={sharePollLink}
        onOpenChange={() => {
          setSharePollLink("");
        }}
